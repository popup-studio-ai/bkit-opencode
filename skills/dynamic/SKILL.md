...
Follow the matching action below. If no action specified, show guide.
...
1. **Ask backend approach** using AskUserQuestion:
   - question: "How would you like to handle the backend?"
   - header: "Backend"
   - options:
     - label: "BaaS (bkend.ai)" / description: "No server to manage. Auth, DB, storage provided as a service. Fast MVP development. (Recommended)"
     - label: "Custom backend" / description: "Build your own server (Node.js/Express, Python/FastAPI, etc.). Full control over backend logic."
     - label: "Other BaaS" / description: "Use another BaaS platform (Supabase, Firebase, etc.)"
   - multiSelect: false
2. Create Next.js + Tailwind project structure
3. If BaaS (bkend.ai): Configure bkend.ai MCP (opencode.jsonc) + src/lib/bkend.ts client template
   If Custom backend: Create server/ directory structure + API route templates
   If Other BaaS: Ask which platform, then configure accordingly
4. Create opencode.jsonc config (Level: Dynamic specified)
5. Create docs/ folder structure
6. Initialize docs/.bkit-memory.json
...
│   └── 04-report/
│
├── .opencode/opencode.jsonc     # OpenCode config (MCP, agents, etc.)
...
|------|--------|-------------|
| developer | baas-expert | Do, Act |
| qa | qa-monitor, gap-detector | Check |

**To use (always available in OpenCode):**
1. Start team mode: `/pdca team {feature}`
2. Monitor progress: `/pdca team status`
