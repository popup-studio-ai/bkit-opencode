...
1. After config, restart OpenCode
2. First connection attempt shows `needs_auth` status
3. Run `opencode mcp auth bkend` in terminal (from the project directory if project-level config)
4. Browser opens for OAuth 2.1 + PKCE authentication
5. After auth, OpenCode reconnects automatically

- No API Key/env vars needed
- Access Token: 1 hour, Refresh Token: 30 days
- OpenCode OAuth redirect URI: `http://127.0.0.1:19876/mcp/oauth/callback`

### Known Issue: invalid_redirect_uri

bkend.ai must whitelist OpenCode's redirect URI (`http://127.0.0.1:19876/mcp/oauth/callback`).
If `invalid_redirect_uri` error occurs, bkend.ai server-side support for OpenCode is required.
Each AI tool has its own OAuth callback port.
...
| MCP tools not visible | Connection lost | Restart OpenCode, check opencode.jsonc |
| 405 SSE error | Normal fallback | StreamableHTTP failed first, check OAuth status |
| invalid_redirect_uri | OpenCode URI not whitelisted | bkend.ai must allow 127.0.0.1:19876 |
...
- **Setup Guide (OpenCode)**: https://raw.githubusercontent.com/popup-studio-ai/bkend-docs/main/en/ai-tools/05-opencode-setup.md
