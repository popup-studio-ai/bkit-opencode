...
|------|---------|----------|
| Find type errors, unused vars, missing imports | LSP diagnostics | Grep patterns |
| Trace function/class definitions | LSP go-to-definition | Grep + Read |
| Find all usages of a symbol | LSP references | Grep for name |
| Understand type signatures | LSP hover | Read source |
| Search for declarations | LSP workspace symbols | Glob + Grep |

**Why**: LSP gives compiler-accurate results. Text search can miss aliased references or produce false positives from comments/strings. Start with LSP; fall back to Grep/Glob only when LSP is unavailable or doesn't cover the use case.
