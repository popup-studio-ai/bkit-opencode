...
|------|---------|----------|
| Find errors to fix | LSP diagnostics | Grep for error patterns |
| Navigate to fix locations | LSP go-to-definition | Grep + Read |
| Check fix side-effects | LSP references | Grep for symbol name |
| Verify fix resolved the issue | LSP diagnostics (re-check) | Re-run evaluator |

**Why**: During iteration, precision matters — LSP tells you exactly what's broken and where, reducing wasted fix attempts. Always check LSP diagnostics after each fix to catch regressions immediately.
...
Analysis/iteration output MUST go to `docs/03-analysis/`, NOT `03-verify` or `03-check`.
...
pdca-iterator automatically tracks iterations with OpenCode's Task System:
...
Plan     → Plan docs created
Design   → Implementation specs defined (includes user journey flows)
Do       → Code implemented
